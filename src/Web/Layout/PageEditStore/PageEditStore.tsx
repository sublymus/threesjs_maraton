...
export function PageEditStore() {
    const [id] = useState(generateUid());
    const { current, setAbsPath, navBack, json, pathList } = useWebRoute();
    const { deleteStore, editStore, selectedStore, openChild, setStoreById, owner, exist } = useWebStore();
                ...
                <div className="center-right">
                    ...
                    <div className="btm">
                        <div className="btn no-selectable" style={{ background: canEnd ? '' : '#345' }} onClick={() => {
                            ...
                            {
                                loading ? <div className="loading"></div> : `Edit Store`
                            }
                        </div>
