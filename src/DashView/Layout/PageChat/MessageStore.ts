...
import { ListenDiscussion, useDiscussionStore } from "./Discussion/DiscussionStore";
...
interface DiscussionState {
    ...
    fetchMessages(context_id: string): Promise<ListType<Message> | undefined>
...
export const useMessageStore = create<DiscussionState>((set) => ({
    ...
    async fetchSendMessage({ context, context_name, files, text }) {
        ...
        if (context.id.startsWith(NEW_DISCUSSION_STR)) {
            ...
            const s = await useSessionStore.getState().fetchCreateSession(context.other.id);
            console.log('S', s);
            if (!s) return
            context = s;
    ...
    async fetchMessages(context_id) {
        const h = useRegisterStore.getState().getHeaders();
        if (!h) return
        const response = await fetch(`${Host}/get_messages/?context_id=${context_id}`, {
            headers: h.headers,
        });
        try {
            const json = await response.json();
